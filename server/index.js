...
app.get('/api/organizations', async (req, res) => {
  try { res.json(await dFetch(`${BASE}/api/v41/organizations?limit=100`)); }
  catch(e) { res.status(500).json({ error: e.message }); }
});

app.get('/api/organizations/:orgId/reports', async (req, res) => {
  try { res.json(await dFetch(`${BASE}/api/v4/organizations/${req.params.orgId}/reports?limit=100`)); }
  catch(e) { res.status(500).json({ error: e.message }); }
});

app.get('/api/organizations/:orgId/reports/:reportId/expenses', async (req, res) => {
  try { res.json(await dFetch(`${BASE}/api/v41/organizations/${req.params.orgId}/reports/${req.params.reportId}/expenses`)); }
  catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/organizations/:orgId/users/:userId/expenses', async (req, res) => {
  try {
    ...
    res.json(await dFetch(`${BASE}/api/v41/organizations/${req.params.orgId}/users/${req.params.userId}/expenses?${params}`));
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.post('/api/organizations/:orgId/expenses', async (req, res) => {
  try { res.json(await dFetch(`${BASE}/api/v4/organizations/${req.params.orgId}/expenses`, 'POST', req.body)); }
  catch(e) { res.status(500).json({ error: e.message }); }
});

app.put('/api/organizations/:orgId/expenses/:expId', async (req, res) => {
  try { res.json(await dFetch(`${BASE}/api/v4/organizations/${req.params.orgId}/expenses/${req.params.expId}`, 'PUT', req.body)); }
  catch(e) { res.status(500).json({ error: e.message }); }
});

app.put('/api/organizations/:orgId/expenses/:expId/report', async (req, res) => {
  try { res.json(await dFetch(`${BASE}/api/v4/organizations/${req.params.orgId}/expenses/${req.params.expId}`, 'PUT', { report: req.body.reportId })); }
  catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/organizations/:orgId/expenses/:expId/resources', async (req, res) => {
  try {
    ...
    form.append('creation_date', new Date().toISOString().split('T')[0]);
    ...
    const text = await r.text();
    try { res.status(r.status).json(JSON.parse(text)); } catch { res.status(r.status).json({ raw: text }); }
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
Strip commas from numbers. Google invoices -> 'Google Ads'. Meta/Facebook -> 'Meta Ads'.`;
    ...
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ model: 'claude-opus-4-5-20251101', max_tokens: 300, messages: [{ role: 'user', content }] })
    });
...
app.listen(PORT, () => console.log('InvoiceHub running at http://localhost:' + PORT));